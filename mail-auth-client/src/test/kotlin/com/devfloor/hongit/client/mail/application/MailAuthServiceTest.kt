...
import com.devfloor.hongit.client.mail.common.config.MailAuthConfig
...
@SpringBootTest(
    classes = [MailAuthService::class, MailAuthConfig::class, SpringTemplateEngine::class],
    properties = ["mail.sender", "hongit"]
)
internal class MailAuthServiceTest {
    private lateinit var mailAuthService: MailAuthService
    ...
    @BeforeEach
    internal fun setUp() {
        mailAuthService = MailAuthService(authTokenRepository, mailSender)
    }
    ...
    internal fun `sendAuthenticationMail - 인증 토큰에 따른 메일 발송`() {
        // given
        val userId: Long = 1
        val request = MailSendRequest(userId, "ljy3510@gmail.com")
        given(authTokenRepository.save(any())).willReturn(AuthToken(userId))

        // when
        mailAuthService.sendAuthenticationMail(request)
    }
}
